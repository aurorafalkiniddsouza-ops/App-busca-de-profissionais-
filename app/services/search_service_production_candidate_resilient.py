...
async def process_dataframe_resilient(
    dataframe: pd.DataFrame,
    council: str,
    searched_state: str | None = None,
    retry_attempts: int = 2,
    delay_between_rows_seconds: float = 2.0,
    delay_between_retries_seconds: float = 3.0,
) -> pd.DataFrame:
    results = []
    ...
    for _, row in dataframe.iterrows():
        ...
        for attempt in range(1, retry_attempts + 1):
            last_result = await process_single_search(searched_name, council, searched_state)

            if last_result.final_status != "ERRO NA CONSULTA":
                break

            if attempt < retry_attempts:
                await asyncio.sleep(delay_between_retries_seconds)

        if last_result is not None:
            results.append(last_result.model_dump())

        await asyncio.sleep(delay_between_rows_seconds)
