...
class CorenSPRegistrationPlaywrightTemplateV2Connector(BaseCouncilConnector):
    ...
    async def search_by_registration(self, registration_number: str, state: str | None = None) -> list[dict[str, Any]]:
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                await page.screenshot(path="coren_sp_registration_debug_v2.png", full_page=True)
                with open("coren_sp_registration_debug_v2.html", "w", encoding="utf-8") as f:
                    f.write(html)
                ...
                for row in candidate_rows:
                    ...
                    has_registration_signal = registration_number in upper_text or any(token in upper_text for token in ["ATIVO", "ATIVA", "INATIVO", "INATIVA", "COREN", "INSCRI", "REGISTRO", "ENFERMAG"])
                    if not has_registration_signal:
                        continue

                    parsed = self._parse_result_block(raw_text, registration_number, state)
                    if not parsed.get("found_name") and not parsed.get("status_text"):
                        continue

                    parsed["evidence_url"] = page.url
                    parsed["evidence_note"] = "Resultado capturado por template Playwright V2 de busca por inscrição do COREN-SP."
                    parsed["notes"] = "Conector em modo de homologação por número de inscrição; filtro de cabeçalho do formulário aplicado."
