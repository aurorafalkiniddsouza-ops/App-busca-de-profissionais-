...
class CorenSPRegistrationPlaywrightTemplateV8Connector(BaseCouncilConnector):
    ...
    async def _detect_recaptcha_block(self, page) -> str | None:
        frame_texts = []
        for idx, frame in enumerate(page.frames):
            try:
                text = await frame.locator("body").inner_text(timeout=2000)
                frame_texts.append((idx, frame.url, text))
            except Exception:
                continue

        for idx, url, text in frame_texts:
            upper = (text or "").upper()
            if "RECAPTCHA" in upper or "VERIFICAR" in upper or "COTA GRATUITA DO RECAPTCHA ENTERPRISE" in upper:
                return f"Bloqueio por reCAPTCHA detectado no frame[{idx}] {url}"
        return None
    ...
    async def search_by_registration(self, registration_number: str, state: str | None = None) -> list[dict[str, Any]]:
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                recaptcha_message = await self._detect_recaptcha_block(page)
                if recaptcha_message:
                    print(recaptcha_message)
                    raise RuntimeError(recaptcha_message)

                raise RuntimeError("Consulta não retornou resultado visível e não houve redirecionamento detectável após o submit.")
