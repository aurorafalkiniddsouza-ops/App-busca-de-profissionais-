...
class CremespPlaywrightTemplateV2Connector(BaseCouncilConnector):
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                if used_submit_selector:
                    ...
                    print("Nenhum botão de submit encontrado. Tentando Enter no campo.")
                ...
                await page.screenshot(path="cremesp_debug_v2.png", full_page=True)
                with open("cremesp_debug_v2.html", "w", encoding="utf-8") as f:
                    f.write(html)
                ...
                for row in candidate_rows:
                    ...
                    if full_name.upper().split()[0] not in upper_text and full_name.upper().split()[-1] not in upper_text:
                        continue
                    ...
                    for line in lines:
                        ...
                        if any(token in upper_line for token in ["CRM", "REGISTRO", "INSCRI"]):
                            registration_number = line
                        if any(token in upper_line for token in ["ATIV", "INATIV", "CANCEL", "SUSPENS", "REGULAR", "IRREGULAR"]):
                            status_text = line
                    ...
                    results.append(
                        {
                            ...
                            "evidence_note": "Resultado capturado por template Playwright V2 do CREMESP.",
                            "notes": "Conector em modo de homologação; revisar seletores e parsing conforme HTML real.",
                        }
                    )
