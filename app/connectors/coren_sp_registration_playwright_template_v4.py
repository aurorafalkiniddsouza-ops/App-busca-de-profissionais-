...
class CorenSPRegistrationPlaywrightTemplateV4Connector(BaseCouncilConnector):
    ...
    async def search_by_registration(self, registration_number: str, state: str | None = None) -> list[dict[str, Any]]:
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                try:
                    await page.wait_for_function(
                        ...
                        () => {
                            ...
                            return bodyText.includes("ATIVO")
                                || bodyText.includes("INATIVO")
                                || bodyText.includes("REGULAR")
                                || bodyText.includes("IRREGULAR")
                                || bodyText.includes("COREN")
                                || bodyText.includes("Inscrição")
                                || bodyText.includes("inscrição");
                        }
                        """,
                        timeout=8000,
                    )
                    print("Mudança de conteúdo detectada após a consulta")
                except Exception:
                    print("Nenhuma mudança explícita de conteúdo detectada após a consulta")
                ...
                await page.screenshot(path="coren_sp_registration_debug_v4.png", full_page=True)
                with open("coren_sp_registration_debug_v4.html", "w", encoding="utf-8") as f:
                    f.write(html)
                ...
                for row in candidate_rows:
                    ...
                    parsed["evidence_note"] = "Resultado capturado por template Playwright V4 de busca por inscrição do COREN-SP."
                    parsed["notes"] = "Conector em modo de homologação por número de inscrição; critério explícito e eventos de submit reforçados."
