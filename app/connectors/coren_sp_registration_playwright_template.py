...
class CorenSPRegistrationPlaywrightTemplateConnector(BaseCouncilConnector):
    ...
    async def search_by_registration(self, registration_number: str, state: str | None = None) -> list[dict[str, Any]]:
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                used_input_selector = await self._fill_first_visible(page, registration_input_selectors, registration_number)
                print("Campo usado:", used_input_selector)
                print("Inscrição preenchida:", registration_number)
                ...
                await page.screenshot(path="coren_sp_registration_debug.png", full_page=True)
                with open("coren_sp_registration_debug.html", "w", encoding="utf-8") as f:
                    f.write(html)
                ...
                for row in candidate_rows:
                    ...
                    if registration_number not in upper_text and "COREN" not in upper_text and "INSCRI" not in upper_text and "REGIST" not in upper_text:
                        continue
                    ...
                    for line in lines:
                        ...
                        if any(keyword in upper_line for keyword in ["ATIV", "INATIV", "SUSPENS", "CANCEL", "BAIXAD", "REGULAR", "IRREGULAR"]):
                            status_text = line
                        if parsed_registration_number is None and any(keyword in upper_line for keyword in ["INSCRI", "REGIST", "COREN"]):
                            parsed_registration_number = line
                    ...
                    results.append(
                        {
                            ...
                            "evidence_note": "Resultado capturado por template Playwright de busca por inscrição do COREN-SP.",
                            "notes": "Conector em modo de homologação por número de inscrição; revisar seletores e parsing conforme HTML real.",
                        }
                    )

                if not results:
                    logger.info("Nenhum resultado localizado para inscrição %s no %s", registration_number, self.council_name)
